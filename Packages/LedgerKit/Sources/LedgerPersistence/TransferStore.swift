...
extension LedgerStore {
    ...
    public func recordTransfer(ledgerId: UUID, from: UUID, to: UUID, amount: Money, occurredAt: Date = Date(), note: String? = nil) throws -> Transfer {
        try writer.write { db in
            try Self.insertTransfer(db, ledgerId: ledgerId, from: from, to: to, amount: amount, occurredAt: occurredAt, note: note, actorId: actorId)
        }
    }

    @discardableResult
    static func insertTransfer(_ db: Database, ledgerId: UUID, from: UUID, to: UUID, amount: Money, occurredAt: Date, note: String?, actorId: UUID) throws -> Transfer {
