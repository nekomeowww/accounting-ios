...
沿用本地 Export As / Save As：CSV 以有效账目和 journal 派生结果为准；完整归档增加 `receiptSchemaVersion`，并按引用闭包导出结构化凭证。

- 导出当前采用的 Receipt Revision，以及 Expense 实际引用的修订；两者不同时一并包含并保留关系。无引用的提取历史默认排除。
- 包含采用的商户、商品、税费、支付、汇总与来源映射；minor unit 输出十进制字符串，DecimalText 原样保留。归档货币 metadata 显式记录所用小数位，以便跨版本验证；不改变本地现有 Money 存储方式。
- 原图仍可选。排除原图时保留 asset ID / availability=excluded，不保留本机路径。所有引用应指向包内对象或明确的外部资源占位。
- API Key、原始模型响应、私有对话、完整证据 rawText、会员 / 客户个人标识默认排除。支付引用与尾号默认省略；必要的 kind / amount / currency 可保留。
- 规范对象可保留 origin 等不敏感 provenance；排除 extraction 时使用无 payload 的来源摘要，不能留悬空的 extractionId / payloadPointer。选项与省略内容写入 manifest。
- 导出只创建副本；导入恢复 UI、持续协作与 CRDT 仍按既定范围后置。
...
| 迁移已有账本与导出不含图片的副本 | 旧余额不变；新表可为空；归档引用完整并明确标记缺失资源 |
