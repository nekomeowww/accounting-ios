...
- V1 支持 Export As / Save As，由本地生成文件；具体格式按下文建议实施。
...
| 文件导出 / 另存为 | 本地 Export Service + 系统分享 / 文件选择器 | 复用相同导出内容，分别选择格式与去向 |
...
2026-09-23 用户确认加入 Export As / Save As。建议先支持 CSV 与结构化账本归档 ZIP，格式细节为本讨论稿建议。

### 9.1 用户流程

账本页面的更多菜单提供“导出…”和“另存为…”；本地编辑始终自动保存，这两个入口创建当前账本在导出时刻的独立副本。

- Export As：选择 CSV / 账本归档与相应选项，生成文件后打开系统分享面板。
- Save As：使用相同格式选择与生成逻辑，随后选择文件名和文件保存位置。
...
CSV 面向表格阅读与后续分析，提供“消费明细 / 成员分摊 / 余额汇总”三种表，一次输出所选表为一个 UTF-8 CSV 文件。

- 消费明细：每笔有效 Expense 一行，含稳定 ID、商户、发生时间与时区、币种、总额、付款人、参与者和分摊方式。
- 成员分摊：每个 Expense × Member 一行，含消费 ID、成员 ID / 名称、币种、分摊金额与付款人 ID。
- 余额汇总：每个 Member × Currency 一行，含垫付、承担和净余额；按本地 Ledger Engine 计算，不混加币种。如果以后加入结算，必须同步扩展汇总字段与净余额定义。
- 显示金额采用无千分位的十进制字符串，并提供 minorUnits / exponent 以保留精度；不得经过 Double 转换。
- 正确转义逗号、引号和换行；对用户 / AI 文本可能触发表格公式的内容做文本化处理，金额字段保持数值语义。
- 当前只导出有效业务记录，不包含 tombstone、操作历史、原图或 AI 原始响应。CSV 是阅读 / 分析格式，不承诺无损恢复账本。

### 9.3 账本归档

建议输出普通 .zip：manifest.json + ledger.json + 可选 assets/。导出的是可迁移的当前业务快照，不直接复制运行中的 SQLite 数据库。
...
- 不导出 API Key、设备偏好、私有对话、AI 原始响应、Action 历史和已删除记录。归档是当前账本副本，不是包含撤销历史的整机备份。
- V1 此项先实现导出和格式往返校验；用户可操作的“导入 / 恢复”入口单独列为后续工作，不将仅能导出描述为已实现备份恢复。

### 9.4 UIKit 与本地实现

Export Service 读取一致的数据库快照，再在后台生成文件；资产生成期间保证所引用文件不被清理。导出完成前不把长时间文件生成放在写事务中。
先写临时目录，完成内容与清单校验后再交给系统。支持进度、取消、磁盘不足与缺失资源错误；取消不留下伪装完整的输出。
临时文件在分享 / 文件导出使用期间保留，结束后按短期清理策略移除；实际清理时机通过设备上的系统分享流程验证。
...
未来导入必须校验格式版本、金额、引用、重复 ID、附件路径和体积，并通过 Domain 写入。需要先明确导入成新账本还是更新已知副本；相同 UUID 不能成为静默覆盖本地账目的理由。
本地副本没有后续更新订阅。Google Sheets / Excel 等外部工具的文件使用路径与持续同步分别设计；当前不实现 OAuth Connector 或自动回写。
Web + Apple 的持续协作继续后置；届时定义成员身份、权限、传输、冲突和附件同步。CRDT 与服务器不作为本阶段前置条件，账务计算继续在客户端完成。
...
- 导出：一致快照、CSV 精度 / 转义 / 公式文本化、归档编码解码后金额与关联不变、附件缺失、取消、磁盘不足、系统目标写入失败与凭据排除。
