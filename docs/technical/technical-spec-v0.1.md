...
- D2（2026-09-23 accepted）：V1 支持 Exact Split，手工表单与 Agent 卡片均可按指定金额分摊，合计必须等于总额。
- D3（2026-09-23 accepted）：账本多币种 + 结算币种折算；支持手工记录还款（transfer），计入余额。
- D4：首个 AI provider / model；用真实且可用于测试的票据集比较提取质量和延迟。
- D5（2026-09-23 accepted）：余额展示简化债务（最少转账笔数）；V1 不设退款类型，退款通过修改或删除原消费表达。
